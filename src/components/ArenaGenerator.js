...
const ArenaGenerator = ({ openAuthModal }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const reshapedResults = response.data.results.map(result => ({
        ...
        image_id: result.id,  // You're already storing the ID here
  ...
  const handleSelectWinner = async (image, index) => {
    const token = localStorage.getItem('token');
    ...
    try {
      ...
      // Use the image_id from the reshaped data
      await axios.post(
              ...
              {generatedImages.map((image, index) => (
                <div 
                  ...
                  className={`bg-white p-4 rounded-lg shadow-md transition-all duration-300 ${
                    selectedWinner === index ? 'ring-4 ring-amber-400 transform scale-102' : ''
                  }`}
                >
                  ...
                  {selectedWinner === null && (
                    <button
                      onClick={() => handleSelectWinner(image, index)}
                  ...
                  {selectedWinner === index && (
                    <div className="text-center text-amber-600 font-bold mt-2">
                      🏆 Winner!
                    </div>
                  )}
                </div>
